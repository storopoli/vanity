...
import qualified Crypto.Bitcoin.Vanity.Worker.CPU as CPU
...
searchWithProgress config callback = case vcBackend config of
  CPU -> CPU.searchVanityWithCallback config callback
#ifdef CUDA_ENABLED
  CUDA -> CUDA.searchVanityWithCallback config callback
#endif
#ifdef WGPU_ENABLED
  WGPU -> WGPU.searchVanityWithCallback config callback
#endif
  backend ->
    error $
